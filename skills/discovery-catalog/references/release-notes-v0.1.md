...
- `cxi-template`에서 만든 canonical skill을 배포용 repo로 분리
...
- 스킬 원본 수정은 `cxi-template`에서 진행하고, `cxi-skill-pack`에서는 직접 수정하지 않는 운영을 권장합니다.

## 운영자용 참고

`cxi-template`에서 다음 명령으로 v0.1 리포트와 배포본을 다시 생성할 수 있습니다.
